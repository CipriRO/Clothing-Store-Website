...
import "../../../components-css/dashboard/products/sizes.css";
...
export default function Sizes({
  ...
  function handleEdit(id) {
    const prevData = sizesList;
    const newData = prevData.map((data) =>
      data.edit === true && data.size_id !== id
        ? { ...data, edit: false }
        : data.size_id === id
        ? { ...data, edit: !data.edit }
        : data
  ...
  async function handleAddSize() {
    ...
    if (form.checkValidity()) {
      ...
      fetch(
        "http://192.168.1.17:8000/src/dashboard-php/products/size/addSize.php",
        {
          method: "post",
          headers: {
        ...
        .then((data) => {
          if (data !== false) {
            setSizesList((prevData) => [
              ...prevData,
              { size_id: data, Name: name, edit: false, delete: true },
            ]);
          }
        });
    }
  ...
  async function handleEditSize(id) {
    const form = editForm.current;
    ...
    if (form.checkValidity()) {
      ...
      fetch(
        "http://192.168.1.17:8000/src/dashboard-php/products/size/editSize.php",
        {
          method: "put",
          headers: {
        ...
        .then((data) => {
          if (data) {
            setSizesList((prevData) =>
              prevData.map((data) =>
                data.size_id === id ? { ...data, Name: name } : data
              )
            );
          }
        });
  ...
  async function handleDelete(id) {
    const response = await fetch(
      "http://192.168.1.17:8000/src/dashboard-php/products/size/deleteSize.php",
      {
        method: "post",
        headers: {
          "Content-Type": "application/json",
    ...
    if (data === true) {
      ...
      setSizesList((prevData) =>
        prevData.filter((data) => data.size_id !== id)
      );
    }
  }
  ...
  return (
    <>
      {sizesState === false || noSizesList === true ? (
        <>
            ...
            {add ? (
              <>
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => handleAddSize(edit.size_id)}
              ...
              <>
                ...
                >
                  {!noSizesList && !sizesState
                    ? "An error has occur!"
                    : "There are no sizes at the moment!"}
                </h6>
        ...
        <>
          <div className="card-body d-flex align-items-center flex-column">
            {edit && !add ? (
              <>
                <p className="text-secondary fw-medium text-center mb-3">
                  Click the size name to cancel editing!
                </p>
                <button
                  onClick={() => handleEdit(edit.size_id)}
                ...
                <div className="d-flex align-items-center mt-3 gap-3">
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => handleEditSize(edit.size_id)}
                      className="btn btn-success fw-medium"
                    >
                      Ok
                    </button>
                  ...
                  <button
                    onClick={() => handleDelete(edit.size_id)}
              ...
              <>
                <p className="text-secondary fw-medium text-center mb-4">
                  The sizes that are in use can't be{" "}
                ...
                <div className="d-flex column-gap-3 row-gap-3 flex-wrap justify-content-center">
                  ...
                  {sizesList.map((size) => (
                    <button
                      key={size.size_id}
                      onClick={() => handleEdit(size.size_id)}
                      className="badge rounded-pill text-bg-secondary fs-6 cursor-pointer fw-medium position-relative border-0"
                    >
                      {size.Name}
                      <span
                        className={`transition-all position-absolute ${
                          size.delete ? "top-30" : "top-0"
